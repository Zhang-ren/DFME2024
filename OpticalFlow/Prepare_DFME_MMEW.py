...
EXPRESSION_VERSION = 'Mix_DFME_MMEW'
VERSION = '22'
...
DFME_PATH = '../MMEW_10'
DFME_SAVE_PATH = '../MMEW_mag_landmarks_10/'
DFME_FLOW_PATH = '../MMEW_mag_motion_flow_10/'
    ...
    for root, dirs, files in os.walk(DFME_PATH):
        for dir in dirs:
            ...
            for dir_path in os.listdir(dir_paths):
                ...
                image_files.sort(key=lambda x:int(x.split('.')[0]))  # 确保文件是按照名称排序的
                
                if image_files and dir != 'repression':
                    onset_pic = image_files[0]
                    middle_image_index = len(image_files) // 2
                    apex_pic = image_files[middle_image_index] if len(image_files) > 1 else first_image
                    ...
                    label_lines.append(str(emotion_dict[dir]) + '\n')
