...
from apexs import apexs,crop_face,imflow,maxrad
# from rad import drawpic
def mix_prepare():
...
#            m.write(Flow_pic_path + '\n')
        flow_files.writelines(Flow_pic_path + '\n')
#        with open (AFFLOW_FILE, 'a') as p:
#            p.write(AFFlow_pic_path + '\n')
        afflow_files.writelines(AFFlow_pic_path + '\n')
#        with open (LABEL_FILE, 'a') as l:
#            l.write(str(row[3]) + '\n')
        label_files.writelines(str(row[3]) + '\n')
#        with open (SUBJECT_FILE, 'a') as s:
#            s.write('samm ' + str(row[1]).zfill(3) + '\n')
        subject_files.writelines('samm ' + str(row[1]).zfill(3) + '\n')
#        with open (DATA_FILE, 'a') as dl:
#            dl.write(save_onset_pic_path + '\n')
#            dl.write(save_apex_pic_path + '\n')
        data_files.writelines(save_onset_pic_path + '\n')
        data_files.writelines(save_apex_pic_path + '\n')

        if not os.path.exists(save_test_path):
            os.makedirs(save_test_path)
        print(pic_onset_path)
        crop_pic(pic_onset_path, pic_apex_path, save_onset_pic_path, save_apex_pic_path, Flow_pic_path, AFFlow_pic_path)

# =================================================================================================

    emotion_dict = {'ne': 'negative', 'po': 'positive', 'sur': 'surprise'}

    SMIC_path = './SMIC_all_raw/'
    ...
    for index, (_, row) in enumerate(SMIC_data.iterrows()):
        ...
        test_path = os.path.join('.'+SMIC_path, subject_name, emotion, test_name)
...
#            d.write(apex_save_path + '\n')
        data_files.writelines(onset_save_path + '\n')
        data_files.writelines(apex_save_path + '\n')
...
#            s.write(SMIC_subject + '\n')
        subject_files.writelines(SMIC_subject + '\n')
...
#            p.write(AFFlow_pic_path + '\n')
        ...
        crop_pic(onset_path, apex_path, onset_save_path, apex_save_path, Flow_pic_path, AFFlow_pic_path)
...
# ===================================================================================================
    ...
    for index, row in CASMEII_data.iterrows():
        ...
        Flow_pic_path = os.path.join(CASMEII_flow_test_path, 'motion_flow.jpg')
...
#            d.write(CASMEII_apex_save_path + '\n')
        data_files.writelines(CASMEII_onset_save_path + '\n')
        data_files.writelines(CASMEII_apex_save_path + '\n')

#        with open (LABEL_FILE, 'a') as l:
#            l.write(str(label) + '\n')
        label_files.writelines(str(label) + '\n')

#        with open (SUBJECT_FILE, 'a') as s:
#            s.write(CASMEII_subject + '\n')
        subject_files.writelines(CASMEII_subject + '\n')

        AFFlow_pic_path = os.path.join(CASMEII_flow_test_path, 'Merge.jpg')

        if not os.path.exists(CASMEII_flow_test_path):
            os.makedirs(CASMEII_flow_test_path)

        crop_pic(CASMEII_onset_path, CASMEII_apex_path, CASMEII_onset_save_path, CASMEII_apex_save_path, Flow_pic_path,AFFlow_pic_path)

#        with open (FLOW_FILE, 'a') as m:
#            m.write(Flow_pic_path + '\n')
        flow_files.writelines(Flow_pic_path + '\n')
#        with open (AFFLOW_FILE, 'a') as p:
#            p.write(Flow_pic_path + '\n')
        afflow_files.writelines(AFFlow_pic_path + '\n')
...
#                        l.write(str(CK_dict[label]) + '\n')
                    label_files.writelines(str(CK_dict[label]) + '\n')
                    ...
                    apex_num = apexs(test_image_path,image_dir,1,1)
...
#                        s.write('Macro\n')
                    ...
                    Flow_pic_path = os.path.join(test_flow_path, 'flow.jpg')
                    AFFlow_pic_path = os.path.join(test_flow_path, 'Merge.jpg')
                    ...
                    crop_pic(onset_path, apex_path, onset_save_path, apex_save_path, Flow_pic_path, AFFlow_pic_path)
                    rad = maxrad(onset_save_path, apex_save_path)
                    rads.append(rad)
...
#    onset_crop = onset_crops[0][:280, :280]
#    apex_crop = apex_crops[0][:280, :280]
